...
export default function DestinationDetails({ destination, }: Props) {
    return (
        <section className="mt-4 rounded-[28px] border border-[#123c35]/10 bg-white p-6 sm:p-8">
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                    ...
                    <p className="mt-4 text-sm leading-7 text-[#5f6e68]">
                        {destination.history}
                    </p>
                </div>
            ...
            <div className="mt-8 grid gap-4 border-t border-[#123c35]/8 pt-6 sm:grid-cols-2">
                <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                        {destination.highlights.map((item) => (
                            <span key={item} className="rounded-full bg-[#e8f58d]/60 px-3 py-1.5 text-[11px] font-bold text-[#31544d]">
                                {item}
                            </span>
                        ),
                        )}
                ...
                <div>
                    ...
                    <p className="mt-3 text-sm font-bold text-[#5f6e68]">
...
function InfoBox({ icon: Icon, title, items, }: { icon: React.ComponentType<{ className?: string; }>; title: string; items: string[]; }) {
    return (
        <div className="rounded-[20px] bg-[#f7f3ea] p-4">
