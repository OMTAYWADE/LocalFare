...
export default function FoodCard({ food, expanded, onToggle, }: FoodCardProps) {
    const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=` + `${food.mapLatitude},${food.mapLongitude}`;

    return (
        <article className=" overflow-hidden rounded-[26px] border border-[#123c35]/10 bg-white transition hover:shadow-[0_16px_45px_rgba(18,60,53,0.07)] ">
            <button type="button" onClick={onToggle} className="w-full p-5 text-left sm:p-6">
                <div className="flex gap-4">
                    {/* Food visual */}

                    <div className="relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[20px] bg-[#f7f3ea] sm:h-28 sm:w-28">
                        {food.image ? (
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                            ...
                            <ChevronDown className={` h-5 w-5 shrink-0 text-[#6d7974] transition-transform ${expanded ? "rotate-180" : ""}`}
                            />
                        </div>
            ...
            {expanded && (
                <div className="border-t border-[#123c35]/8 bg-[#f7f3ea]/50 px-5 pb-5 pt-5 sm:px-6 sm:pb-6">
                    <p className="text-sm leading-6 text-[#5f6e68]">
                        {food.description}
                    </p>
                    ...
                    <div className="mt-3 rounded-[18px] bg-white p-4">
                        ...
                        <div className="mt-3 space-y-2">
                            {food.priceEvidence.map(
                                (evidence) => (
                                    <p
                                        key={evidence}
                                        className="text-xs leading-5 text-[#6d7974]"
                                    >
                                        • {evidence}
                                    </p>
                                ),
                        ...
                        <div className="mt-4">
                            <FreshnessBadge status={ food.confidence.freshness} lastUpdated={ food.confidence.lastUpdated} />
                        </div>

                        <p className="mt-2 text-[11px] text-[#6d7974]">
                            {food.confidence.score}% confidence
                            from{" "}
                            {food.confidence.sourceCount}{" "}
                            sources.
                        </p>
                    </div>
                    ...
                    <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer" className=" mt-4 flex h-12 items-center justify-center gap-2 rounded-full bg-[#123c35] text-xs font-black text-white transition hover:bg-[#0d312b]">
