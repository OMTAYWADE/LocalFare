...
export default function TripBudgetCard({ expenses, maximumBudget }: TripBudgetCardProps) {
  ...
  return (
    <section className="mt-8">
      <div className="overflow-hidden rounded-[30px] bg-[#123c35] text-white shadow-[0_22px_60px_rgba(18,60,53,0.16)] transition-shadow duration-300 hover:shadow-[0_28px_70px_rgba(18,60,53,0.22)]">
        <button
          type="button"
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-md text-xs leading-5 text-white/50">
                A practical upper estimate for the expenses we currently know about.
              </p>
            </div>

            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 transition-transform duration-300 hover:bg-white/15">
            ...
            <div className="border-t border-white/10 px-6 pb-6 sm:px-8 sm:pb-8">
              <div className="divide-y divide-white/10">
                {expenses.map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between gap-5 py-4">
                    <div>
                      <p className="text-sm font-bold text-white">{categoryLabels[expense.category]}</p>
                      <p className="mt-1 text-[11px] text-white/40">{expense.description}</p>
                    </div>
